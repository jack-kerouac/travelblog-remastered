...
- **Layouts**: `layouts/` directory contains:
  ...
  - `shortcodes/video.html` - HTML5 video embed shortcode, used as `{{</* video src="file.mp4" */>}}`
...
The map reads `lat` and `lng` from child page frontmatter. Coordinates are set via `entry_property_overrides.json` in the `travelblog-resampling` repo and applied during markdown conversion.
